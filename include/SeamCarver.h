...
private:
    ...
    {
        double sum = 0;
        size_t prev_coordinate = 0;
    };

    std::vector<std::vector<Node>> BuildHorizontalTable() const;

    size_t FindEndCoordinateOfHorizontalSeam(const std::vector<std::vector<Node>> &) const;
};
